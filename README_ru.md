...
```bash
# Распаковка архива .rpa. Результат будет в папке `02_Output_RPA`.
...
# Копирование всех файлов .rpa из папки с игрой в `01_Input_RPA`. Можно также копировать .rpyc (в `03_Input_RPYC`) или .rpy (в `04_Output_RPYC`).
main.py pull ~/Games/SomeVisualNovel rpa

# Перемещение всех файлов .rpyc из `02_Output_RPA` в `03_Input_RPYC`. Можно также перемещать .rpy (в `04_Output_RPYC`) или оба типа (в соответствующие папки).
main.py move rpyc
